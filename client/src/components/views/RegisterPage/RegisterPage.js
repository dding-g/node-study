...
function RegisterPage(props) {
	...
	const emailValid = (event) => {
		...
		axios.post('/api/users/register/emailValid', body)
		.then((response) => {
        ...
        <div className="text-center sing-in-body">
            <form className="form-signin" onSubmit={onRegisterHandler}>
				...
				<div style={{display:'flex', flexDirection:'row'}}>
					...
					<button className='btn btn-default check-btn' type="button" onClick={emailValid}>Check</button>
				</div>
				
                <input type="password" id="inputPassword" className="form-control" placeholder="비밀번호" value ={Password} onChange={onPasswordHandler}/>
				
				<input type="password" id="inputPasswordCheck" className="form-control" placeholder="비밀번호 확인" onChange={onCheckPasswordHandler}  />
				<div class="left-text" id="passwordCheckText">{PasswordCheckText}</div>
				
				<a href = "login">로그인</a>
				
                <button className="btn btn-lg btn-primary btn-block" type="submit" >회원가입 하기</button>
            </form>
        </div>
    )
}
